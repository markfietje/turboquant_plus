...
llama_model_gemma4::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // keep all rows when extracting pre-norm embeddings (MTP target needs the hidden state for every token)
        if (il == n_layer - 1 && inp_out_ids && !cparams.embeddings_pre_norm) {
    ...
    // Expose the pre-norm hidden state (the LM-head input feature) so that
    // MTP draft contexts can read it via llama_get_embeddings_pre_norm_ith() as the
